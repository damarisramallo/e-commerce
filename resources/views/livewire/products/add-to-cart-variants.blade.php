                            ...
                            <ul class="flex items-center space-x-4">
                                @foreach ($option->pivot->features as $feature)
                                    <li>
                                        @switch($option->type)
                                            @case(1)
                                                <button class="w-20 h-8 font-semibold uppercase text-sm rounded-lg {{ $selectedFeatures[$option->id] == $feature['id'] ? 'bg-emerald-600 text-white' : 'border border-gray-400 text-gray-700 shadow-md' }}"
                                                wire:click="$set('selectedFeatures.{{$option->id}}', {{$feature['id']}})">
                                                    {{$feature['value']}}
                                                </button>
                                                @break
                                            @case(2)
                                                <div class="p-0.5 border-2 rounded-lg flex items-center -mt-1 {{$selectedFeatures[$option->id] == $feature['id'] ? 'border-emerald-700' : 'border-transparent'}}">
